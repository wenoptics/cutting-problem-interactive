...
import {ALLOW_RIGHT_ENDS, CuttingSequence, EndType, TargetShape} from "~/components/types";
...
class MaterialState {
    ...
    public get currentWasteFromAdapterCuts(): number {
        return this._assignedSequence.reduce((acc: number, cur: CuttingSequence[0]) => {
            if (isAdapterCut(cur)) {
